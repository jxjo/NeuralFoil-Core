...
It provides a fit_cst helper to fit a CST representation to sampled airfoil coordinates, 
to evaluate polars with NeuralFoil.

This module is self-contained - It only depends on NumPy and the Python standard library.
"""
...
def fit_cst(
    ...
    Args:
        ...
        le_mode: Leading-edge mode controlling the treatment of the leading-edge
            coefficient ``a0``. Supported values are ``"free"``, ``"c2"``.
    ...
    Returns:
        A dictionary with NeuralFoil-compatible Kulfan keys:
    ...
    if n_weights < 2:
        raise ValueError("fit_cst: n_weights must be >= 2")

    if le_mode not in {"free", "c2"}:
        raise ValueError("fit_cst: le_mode must be 'free' or 'c2'")
    if smooth_lambda < 0.0:
        raise ValueError("fit_cst: smooth_lambda must be >= 0")
    ...
    if np.any((x_u < 0.0) | (x_u > 1.0)):
        raise ValueError("fit_cst: x_upper must be within [0, 1]")
    if np.any((x_l < 0.0) | (x_l > 1.0)):
        raise ValueError("fit_cst: x_lower must be within [0, 1]")
    if not y_u[0] == 0.0:
        raise ValueError("fit_cst: y_upper[0] must be 0")
    if not y_l[0] == 0.0:
        raise ValueError("fit_cst: y_lower[0] must be 0")
    if not np.isclose(y_u[-1] + y_l[-1], 0.0):
        raise ValueError("fit_cst: y_upper[-1] + y_lower[-1] must be 0")
    ...
    return {
        "upper_weights": weights_upper,
        "lower_weights": weights_lower,
...
class CST:
    ...
    def te_gap(self) -> float:
        """Return the trailing-edge gap of this single CST curve."""
        return self._te_gap
    ...
    def eval_y (self, u: float | ArrayLike, der: int = 0) -> float | np.ndarray:
        """Evaluate ``y`` or one of its derivatives."""

        x = self._as_x_array(u)

        if der == 0:
            y = self._eval_1D(x, der=0)
        elif der == 1:
            y = self._eval_1D(x, der=1)
        elif der == 2:
            y = self._eval_1D(x, der=2)
        else:
            raise ValueError("CST.eval_y: der must be 0, 1 or 2")
